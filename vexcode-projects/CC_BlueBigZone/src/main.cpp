...
void DeployArms(float timeSecs)
{
  ...
  /*
  Brain.resetTimer();
  while(Brain.Timer < timeSecs * 1000)
  {
     Ramp.spin(vex::directionType::fwd, 90, vex::velocityUnits::pct);
  }
  if(Brain.Timer >= timeSecs * 1000) {
     Ramp.stop();
  }
  */
}
...
void stack()
{
  ...
  //Ramp.resetRotation();
  //Ramp.rotateFor((45 * 2), vex::rotationUnits::deg, 40, vex::velocityUnits::pct, false);
  Ramp.spin(vex::directionType::rev, 100, vex::velocityUnits::pct);
  vex::task::sleep(2400);
  Ramp.stop();
  vex::task::sleep(500);
  BR.spin(vex::directionType::fwd, 50, vex::percentUnits::pct);
